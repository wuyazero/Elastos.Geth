...
The code tutorial [p2p programming in Elastos.Geth](https://github.com/nolash/Elastos.Geth-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `Elastos.Geth` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `Elastos.Geth` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
